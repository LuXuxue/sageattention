...
env_config_json = os.environ.get('COMFY_KITCHEN_CONFIG_JSON')
print_autotuning = os.environ.get("TRITON_PRINT_AUTOTUNING", "0").lower() in ("1", "true", "yes")
if env_config_json:
    env_config = json.loads(env_config_json)
    config_dict = {
        'block_m': env_config.get('BLOCK_M'),
        'block_n': env_config.get('BLOCK_N'),
        'block_k': env_config.get('BLOCK_K'),
        'group_size_m': env_config.get('group_size_m'),
    }
    if arch.startswith("gfx"):
        config_dict['waves_per_eu'] = env_config.get('waves_per_eu', None)
    configs = [
        triton.Config(
            config_dict,
    ...
    configs = [
        triton.Config({'block_m': 64, 'block_n': 64, 'block_k': 128, 'group_size_m': 4, 'waves_per_eu': 1}, num_warps=2, num_stages=2), #gfx1103
    ]
elif arch.startswith("gfx10"):
    configs = [
        triton.Config({'block_m': 64, 'block_n': 128, 'block_k': 64, 'group_size_m': 8, 'waves_per_eu': 1}, num_warps=2, num_stages=2), #gfx1035
    ]
else:
    configs = [
...
def int8_linear(
    ...
    if print_autotuning:
        autotune_func = _int8_matmul_dequant_per_row_kernel if is_per_channel else _int8_matmul_dequant_kernel
        best_config = getattr(autotune_func, 'best_config', None)
        if best_config is not None:
            if hasattr(best_config, 'kwargs'):
                config_kwargs = best_config.kwargs
            elif hasattr(best_config, 'all_kwargs'):
                config_kwargs = best_config.all_kwargs()
            else:
                config_kwargs = {}
            block_m = config_kwargs.get('block_m', 'N/A')
            block_n = config_kwargs.get('block_n', 'N/A')
            block_k = config_kwargs.get('block_k', 'N/A')
            waves_per_eu = config_kwargs.get('waves_per_eu', 'N/A')
            num_warps = getattr(best_config, 'num_warps', 'N/A')
            num_stages = getattr(best_config, 'num_stages', 'N/A')
            print(f"[ComfyKitchen Autotune] Input Shape: M={m}, N={n}, K={k}, is_per_channel:{is_per_channel} | "
                  f"Best Config -> block_m:{block_m}, block_n:{block_n}, block_k:{block_k}, group_size_m:{group_size_m}, "
                  f"waves_per_eu:{waves_per_eu}, num_warps:{num_warps}, num_stages:{num_stages}")
